...
const SeismoEngine = {
    ...
    init() {
        ...
        this.state.map = new mapboxgl.Map({
            ...
            center: [35, 39],
            zoom: 2,
            projection: 'globe', // Dünyayı aktif eder
            antialias: true
        });
        ...
        // Kullanıcı Hakimiyeti: Dokunma ve hareket algılama
        const stopRotation = () => {
            ...
            }, 5000); // Bıraktıktan 5 saniye sonra tekrar döner
        };
    ...
    startRotationLoop() {
        const rotate = () => {
            if (this.state.isRotating && !this.state.userInteracting) {
                ...
                center.lng -= 0.1; // Daha yumuşak dönüş
...
SeismoEngine.attachEventHandlers = function() {
    ...
    map.on('click', 'unclustered-point', (e) => {
        ...
        new mapboxgl.Popup({ offset: 15 })
            ...
            .setHTML(`
                <div class="popup-card" style="color: #333;">
                    <div style="background: ${this.getMagColor(props.mag)}; color: #fff; padding: 5px; border-radius: 4px;">
                        <strong>M<sub>w</sub> ${props.mag.toFixed(1)}</strong>
                    </div>
                    <p><strong>Bölge:</strong> ${props.place}</p>
                    <p><strong>Derinlik:</strong> ${props.depth > 0 ? props.depth + ' km' : 'Sığ'}</p>
                </div>`).addTo(map);
    });
